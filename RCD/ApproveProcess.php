...
if (isset($_SESSION['StaffId']) && $_SESSION['StaffType'] === 'RCD') {
    ...
    if (isset($_REQUEST['Document'])) {
        $stmt = $conn->prepare("select DISTINCT * from documents as d, Proposal as p where d.Proposal_ID=p.ID and d.Rcc_Status='Approved'   and p.Status='On Progress' and d.Status!='Completed' and p.Status='Merged and p.  and  p.Type=? and p.Faculty=? and Faculty_Level='Approved' and p.Department=? order by date ASC");
        $Type = $_REQUEST["v"];
    ...
    if (isset($_REQUEST['Approve'])) {
        ...
        $stmt = $conn->prepare("select * from proposal where Department_level='Approved' and Rcc_level='Approved' and Status!='Completed' and Status!='On Progress' and Status!='Merged' and Faculty_Level='Approved'  and date like '" . $Year . "%' and Type=? and Faculty=? and Department=?  order by date ASC");
        $Type = $data[0];
        $Department = $data[1];
        $Faculty = $data[2];
        ...
        // echo " department ". $Department . " faculty =".$Faculty." type = ".$Type;
        if ($result->num_rows) {
                    ...
                    <td>
                        <center>
                            <div class="form-group">
                                <div class="col-lg-12">
                                    <select class="form-control" id="Type" onchange="ChangeType(this.value)" name="Type" required>
                                        <option value="">Select Type</option>
                                        <option value="Approve&&data=<?php echo $data['Type'] ?>/<?php echo $data['Department'] ?>/<?php echo $data['Faculty'] ?>/<?php echo $data['ID'] ?>">Approve</option>
                                        <option value="Reject&&data=<?php echo $data['Type'] ?>/<?php echo $data['Department'] ?>/<?php echo $data['Faculty'] ?>/<?php echo $data['ID'] ?>">Reject</option>
                                        <option value="suspend&&data=<?php echo $data['Type'] ?>/<?php echo $data['Department'] ?>/<?php echo $data['Faculty'] ?>/<?php echo $data['ID'] ?>">Suspend</option>
                                    </select>
                                </div>
                            </div>
                            <!-- <div class="dropdown">
        ...
        $stmt = $conn->prepare("SELECT DISTINCT s.ID,s.First_Name,s.Middle_Name,s.Last_Name,p.ID,p.Title,p.File,pa.Role,pa.Agreement,p.Faculty_Agreement_Status,p.RCD_Agreement_Status,pa.Staff_ID,p.Cost,p.RCC_Agreement_Status FROM participant as pa INNER JOIN proposal as p ON pa.Proposal_ID = p.ID INNER JOIN staffs as s  ON s.ID=pa.Staff_ID and p.RCC_Agreement_Status='Approved' and p.Faculty_Agreement_Status='Approved' and p.Status!='Completed' and p.Status!='Rejected' and p.Status!='Suspended' and p.Type=? and p.Department=? and p.Faculty=? and date like ?   ORDER BY pa.Role ASC ,p.Status ASC");
        $year = Date("Y") . '%';
        $Type = $data[0];
        $Department = $data[1];
        $Faculty = $data[2];
        // echo $Type . ' ' . $Faculty . ' ' . $Department;
        $stmt->bind_param("ssss", $Type, $Department,$Faculty, $year);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($result->num_rows) {
            ...
            ?>
                   <tr>
                       ...
                       <td>
                           <center><?php echo $data["RCD_Agreement_Status"] ?></center>
                       </td>
        ...
        $stmt = $conn->prepare("select DISTINCT * from Proposal as p where   p.Status!='On Progress' and  p.Status!='Completed' and p.Department_level='Approved' and p.Rcc_level='Approved' and p.Rcd_level!='Rejected' and  p.Type=? and p.Faculty=? and p.Department=? order by p.Rcc_level ASC");
        $Type = $_REQUEST["v"];
        $Faculty = $_REQUEST["Faculty"];
        $Department = $_REQUEST["Department"];
        // echo $Type . ' ' . $Faculty . ' ' . $Department;
                ...
                <tr>
                    ...
                    <td>
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                <li>
                                    <a href="Transfer.php?Transfer=Transfer&&Action=Research&&ProposalId=<?php echo $data['ID'] ?>&&Type=<?php echo $data['Type'] ?>">Research</a>
                                </li>
                                <li>
                                    <a href="Transfer.php?Transfer=Transfer&&Action=Technology Transfer&&ProposalId=<?php echo $data['ID'] ?>&&Type=<?php echo $data['Type'] ?>">Technology Transfer</a>
                                </li>
                                <li>
                                    <a href="Transsfer.php?Transfer=Transfer&&Action=Comunity Service&&ProposalId=<?php echo $data['ID'] ?>&&Type=<?php echo $data['Type'] ?>">Community Service</a>
                                </li>
                            </ul>
                        </div>
                    </td>
        ...
        $data = explode("/", $_REQUEST["data"]);
        if (count($data) === 4) {
            $stmt = $conn->prepare("select * from proposal where  Status!='On Progress' and Status!='Completed' and Status!='Merged' and Department_level='Approved' and Rcc_level='Approved' and  Department=?  and Type=? and date like '" . $Year . "%' order by date DESC");
            $Type = $data[0];
            $Department = $data[1];
            $Faculty = $data[2];
            $Type = $data[0];
                                            ...
                                            <div class="modal-body">
                                                <div class="col-lg-12">
                                                    ...
                                                    <select class="form-control" onchange="selectDepartement(this.value)" id="Faculty" required>
                                                        ...
                                                        ?>
                                                            <option value="<?php echo $data1['Name'] . '/' . $proposal_Id ?>"><?php echo $data1['Name'] ?></option>
                                                        <?php
                                                        }
                                                        ?>
                                                    </select>
                                                ...
                                                <div class="col-lg-12">
                                                    ...
                                                    <select class="form-control" onchange="selectType(this.value)" id="Department_modal" required>
                                                        <option value="">Select Faculty First</option>
                                                    </select>
                                                </div>
                                                <div class="col-lg-12">
                                                    <label class="control-label">Select Type</label>
                                                    <select class="form-control" onchange="selectProposal(this.value+'/hello')" id="Type_modal" required>
            ...
            $Faculty = $data[2];
            $ProposalID = $data[3];
            $stmt->bind_param("iss", $ProposalID, $Department, $Type);
            $stmt->execute();

            $result = $stmt->get_result();
            if ($result->num_rows > 0) {
                while ($data_modal = $result->fetch_assoc()) {
                ?>
                    <form id="role-form" method="POST" action="Merge_Proposal.php">
                        <div class="form-group">
                            <option value="<?php echo $data_modal['ID'] . "/" . $ProposalID . "/" . $Type . "/" . $Department ?>"><?php echo $data_modal['Title']  ?>(<?php echo $data_modal['ID']  ?>)</option>
                        </div>
                    </form>
                    </div>
                <?php
    ...
    } else if (isset($_REQUEST['Merge_modal'])) {
        $Year = date('Y');
        $data = explode("/", $_REQUEST["data"]);
        $stmt = $conn->prepare("select * from proposal where  Status!='On Progress' and Status!='Completed' and Status!='Merged' and Department_level='Approved' and Rcc_level='Approved' and  Department=?  and Type=? and date like '" . $Year . "%' order by date DESC");
        $Type = $data[0];
                                ...
                                <div class="modal-content">
                                    <form id="role-form" method="POST" action="Merge_Proposal.php">
                                        ...
                                        <div class="modal-body">
                                            <div class="col-lg-4">
                                                <label class="control-label">Select Facult</label>
                                                <select class="form-control" onclick="selectDepartement(this.value)" id="Faculty" required>
                                            ...
                                            <div class="col-lg-4">
                                                <label class="control-label">Select Department</label>
                                                <select class="form-control" onclick="selectType(this.value)" id="Department" required>
                                                    <option value="">Select Faculty First</option>
                                                </select>
                                            </div>
                                            <div class="col-lg-4">
    ...
    }else if (isset($_REQUEST["selectDepRCD"])) {
        ...
        if ($num > 0) { ?>
            ...
            ?>
                ...
                <option value="<?php echo  $data["Name"].'/'.$Faculty; ?>"><?php echo $data["Name"]; ?></option>
            <?php
            }
        } else {
            ?>
    ...
    }else if  (isset($_REQUEST["selectDep"])) {
        $stmt = $conn->prepare("select  * from department where Faculty=?");
        $Faculty = $_REQUEST["Faculty"];
        ...
        <select>
            ...
            <option value="<?php echo "Research," . $Department . "," . $data[1]; ?>">Research</option>
            <option value="<?php echo "Technology Transfer," . $data[0] . "," . $data[1]; ?>">Technology Transfer</option>
            <option value="<?php echo "Community Service," . $data[0] . "," . $data[1]; ?>">Community Service</option>
        </select>
      <?php
    }else if (isset($_REQUEST["selectTypeRCD"])) {
               $data=$_REQUEST["Department"]; 
        ?>
        <option>Select Document Type</option>
        <option value="Research/<?php echo $data; ?>">Research</option>
        <option value="Technology Transfer/<?php echo $data; ?>">Technology Transfer</option>
        <option value="Community Service/<?php echo $data; ?>">Community Service</option>
        <option value="Other/<?php echo $data; ?>">Other</option>
      <?php
    }else if (isset($_REQUEST["selectType"])) {
