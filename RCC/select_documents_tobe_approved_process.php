...
if (isset($_SESSION['StaffId']) && $_SESSION['StaffType'] === 'RCC') {
    ...
    if (isset($_REQUEST["departments"])) {
        ...
        $Year = date('Y');
        $stmt = $conn->prepare("select d.ID,p.Department,d.Proposal_ID,d.Term,d.file,p.Title,p.Type,p.date,d.Rcc_Status from proposal as p, documents as d  where p.ID=d.Proposal_ID and    p.Department=?  and d.Status!='Completed' and p.Status='On Progress' and p.Status!='Compeleted' and p.date like '" . $Year . "%' order by p.ID ASC");
...
?>
                ...
                <tr>
                    ...
                    <td><?php echo $data["Rcc_Status"] ?></td>
                    <td>
